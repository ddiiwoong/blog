...
* [USB Network Native Driver for ESXi](https://flings.vmware.com/usb-network-native-driver-for-esxi?download_url=https%3A%2F%2Fdownload3.vmware.com%2Fsoftware%2Fvmw-tools%2FUSBNND%2FESXi701-VMKUSB-NIC-FLING-40599856-component-17078334.zip)  
  PN50 USB NIC(RTL8153) 인식을 위한 네이티브 드라이버 다운로드
...
관리자 권한으로 PowerShell을 실행하고 PowerCLI 모듈 설치전에 실행 권한을 수정한다. (참고 : [https://docs.microsoft.com/en-us/powershell/module/microsoft.powershell.core/about/about_execution_policies?view=powershell-7.1](https://docs.microsoft.com/en-us/powershell/module/microsoft.powershell.core/about/about_execution_policies?view=powershell-7.1) )
...
stable 릴리스로 구성하기 위해 `ESXi-7.0.1-16850804-standard` 이미지 프로필로 설치를 진행한다.  
...
ESXi 부팅 이미지 제작은 [rufus](https://rufus.ie/), [ventoy](https://www.ventoy.net/en/index.html) 둘 중 편한 도구로 선택하면 되는데 ventoy가 조금 더 활용도가 높다고 판단하여 진행했고, OS설치 USB 생성은 생략하도록 한다. 
...
스크립트 실행은 동일한 PowerShell이 동작하는 Window WSL2 환경에서 수행했기 때문에 WSL환경의 `/etc/hosts` 파일에 아래와 같이 사용하고자 하는 IP주소를 미리 등록해놓는다.  
...
해당 레포의 `deploy_photon_router_ova.sh`를 자신의 환경에 맞게 수정한다. WSL내에서 Window 시스템내에 다운로드한 OVA파일 경로는 `/mnt/c/esxi/photon-hw13_uefi-3.0-a383732.ova`로 변경하였다. Router와 ESXi hostname, password등을 확인하고 Network과 Datastore는 위에서 확인한 내용으로 수정한다.
...
`vcsa-deploy`를 사용하여 배포를 진행한다. 먼저 VCSA 7.0 Update 1 ISO를 다운로드한다.  
...
패스워드, Network와 Datastore를 설정하고 DNS서버를 Router IP로 변경한다. 나머지는 기본값으로 설정한다. 
...
위와 같이 SSL connection 에러가 발생하기 때문에 인증서 체크를 무시하는 설정을 한다. 

```powershell
PS C:\esxi\vsphere-with-tanzu-homelab-scripts> Set-PowerCLIConfiguration -InvalidCertificateAction:Ignore
```
