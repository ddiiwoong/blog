...
Kubernetes GitOps를 구현할수 있는 git-sync에 대해서 알아보자.
...
기존에 storage volume 으로 활용되던 [gitrepo](https://kubernetes.io/docs/concepts/storage/volumes/#gitrepo)가 `deprecated` 되어서 방법을 찾다가 공식 repo에서 우연히 발견하게 되었다.

여러 프로젝트들중에 [git-sync](https://github.com/kubernetes/git-sync)는 sidecar 방식으로 git repository를 clone하는 아주 단순한 프로젝트이다.  
...
[skaffold](https://github.com/GoogleContainerTools/skaffold), [draft](https://draft.sh/)와 같이 별도의 binary으로도 GitOps구현이 가능하겠지만 
간단하게 `sidecar` 방식의 GitOps tool로서 `git-sync`는 충분히 활용이 가능할 것이다.